...
import { DefaultVideoPlaceholder, StreamVideoParticipant, ToggleAudioPreviewButton, ToggleVideoPreviewButton,
...
export const CallLoddy = ({ OnJoin }: Props) => {
  const {useCameraState, useMicrophoneState } = useCallStateHooks();
  const {hasBrowserPermission : hasMicPermission} = useMicrophoneState();
  const {hasBrowserPermission : hasCameraPermission} = useCameraState();

  const hadBrowserPermission = hasMicPermission && hasCameraPermission;
        ...
        <div className="flex flex-col items-center justify-center gap-y-6 bg-background rounded-lg p-10 shadow-sm">
          <div className="flex flex-col gap-y-2 text-center">
            ...
            <VideoPreview DisabledVideoPreview={
              hadBrowserPermission ? DisableVideoPreview : AllowBrowserPermissions
            } />
